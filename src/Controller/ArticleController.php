...
class ArticleController extends AbstractController {
    ...
    public function fixtures(){
        ...
        $arrayCategorie = array('0',"1","2","3","4","5","6",11);
        ...
        for($i = 1;$i <=35; $i++){
            ...
            shuffle($arrayTitre);
            shuffle($arrayStatus);
            shuffle($arrayCategorie);

            $dateajout->modify('+'.$i.' day');

            $article->setTitre($arrayTitre[0]);
            $article->setDescription('On sait depuis longtemps que travailler avec du texte lisible et contenant du sens est source de distractions, et empêche de se concentrer sur la mise en page elle-même. L\'avantage du Lorem Ipsum sur un texte générique comme \'Du texte. Du texte. Du texte.\' est qu\'il possède une distribution de lettres plus ou moins normale, et en tout cas comparable avec celle du français standard. De nombreuses suites logicielles de mise en page ou éditeurs de sites Web ont fait du Lorem Ipsum leur faux texte par défaut, et une recherche pour \'Lorem Ipsum\' vous conduira vers de nombreux sites qui n\'en sont encore qu\'à leur phase de construction. Plusieurs versions sont apparues avec le temps, parfois par accident, souvent intentionnellement (histoire d\'y rajouter de petits clins d\'oeil, voire des phrases embarassantes).');
            $article->setDateAjout($dateajout->format('Y-m-d'));
            $article->setAuteur($arrayAuteur[0]);
            $article->setStatus($arrayStatus[0]);
            $article->setIdcat($arrayCategorie[0]);
